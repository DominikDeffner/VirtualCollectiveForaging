...
stan.dataLagged$t <- stan.dataLagged$t[which(stan.dataLagged$Group %in% 1:2)]
stan.dataLagged$Group <- stan.dataLagged$Group[which(stan.dataLagged$Group %in% 1:2)]
stan.dataLagged$Round <- stan.dataLagged$Round[which(stan.dataLagged$Group %in% 1:2)]
stan.dataLagged$dist <- stan.dataLagged$dist[which(stan.dataLagged$Group %in% 1:2)]
stan.dataLagged$vis <- stan.dataLagged$vis[which(stan.dataLagged$Group %in% 1:2)]
stan.dataLagged$n_exploit <- stan.dataLagged$n_exploit[which(stan.dataLagged$Group %in% 1:2)]
stan.dataLagged$Environment <- stan.dataLagged$Environment[which(stan.dataLagged$Group %in% 1:2)]
stan.dataLagged$Incentives <- stan.dataLagged$Incentives[which(stan.dataLagged$Group %in% 1:2)]
stan.dataLagged$N <- length(stan.dataLagged$t[which(stan.dataLagged$Group %in% 1:2)])
...
fit_parallel <- m_parallel$sample(stan.dataLagged, chains = 2, parallel_chains = 2, threads_per_chain = 40, refresh = 1, iter_warmup = 1000 ,adapt_delta = 0.95, max_treedepth = 11, iter_sampling = 1000)
