...
###
##
# Viterbi algorithm to obtain most likely state sequence based on fitted Social Hidden Markov Decision model
##
###
...
N_samp <- 100
viterbi <- matrix(NA, nrow = stan.data$T, ncol = N_samp )
N <- 2

#Loop over all N_samp samples in the posterior 
for (i in 1:N_samp ) {
  print(i)  
  max_logp <- c()
