...
###
##
# Script to compute behavioral summary statistics for each player/group and round
##
###
####
...
for (id in unique(dat_players$id)) {
  for (round in 1:4) {
    ...
    if(( id %in% c(5,6,7,8,81,82,83,84) & round == 3) == FALSE){
      ...
      #When were exploiting players observed?
...
d_group <- data.frame(id = rep(1:40, each = 4), round = rep(1:4,40), Coins = NA, Exploit = NA, DC = NA,  Distance = NA, Visibility = NA, discoveries = NA, joining = NA, joining_real = NA, scrounging = NA, observed =NA, density = NA, Pay = NA, Env = NA )
...
for (group in unique(d$Group)) {
  for (round in 1:4) {
    ...
    d_group$observed[d_group$id == group & d_group$round == round] <- mean(d$exploit_observed[d$Group == group & d$round == round])
