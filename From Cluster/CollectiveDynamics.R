...
#This script uses the processed dat_players file to compute additional variables used in the computational HMMs
...
##
# Spatial GP Model
# Effect of distance on discoveries
##
###

stan.dataSpatialExplore <- stan.dataLagged 

stan.dataSpatialExplore$Exploit <- ifelse(stan.dataSpatialExplore$n_exploit > 0, 1, 0)

stan.dataSpatialExplore$dist_cat <- sapply(1:stan.dataSpatialExplore$N, function(i) which.min(abs(stan.dataSpatialExplore$dist[i] - seq(1,81,2)))  )

stan.dataSpatialExplore$discovery <- rep(0, stan.dataSpatialExplore$N)


for (t in 1:(stan.dataSpatialExplore$N-1)) {
  print(t)
  if(stan.dataSpatialExplore$Round[t+1] == stan.dataSpatialExplore$Round[t] && stan.dataSpatialExplore$Group[t+1] == stan.dataSpatialExplore$Group[t] && stan.dataSpatialExplore$Exploit[t+1] == 1){
    stan.dataSpatialExplore$discovery[t] = 1
  }
}



stan.dataSpatialExplore$t <- stan.dataSpatialExplore$t[which(stan.dataSpatialExplore$Exploit == 0)]
stan.dataSpatialExplore$Group <- stan.dataSpatialExplore$Group[which(stan.dataSpatialExplore$Exploit == 0)]
stan.dataSpatialExplore$Round <- stan.dataSpatialExplore$Round[which(stan.dataSpatialExplore$Exploit == 0)]
stan.dataSpatialExplore$dist_cat <- stan.dataSpatialExplore$dist_cat[which(stan.dataSpatialExplore$Exploit == 0)]
stan.dataSpatialExplore$discovery <- stan.dataSpatialExplore$discovery[which(stan.dataSpatialExplore$Exploit == 0)]
stan.dataSpatialExplore$Environment <- stan.dataSpatialExplore$Environment[which(stan.dataSpatialExplore$Exploit == 0)]
stan.dataSpatialExplore$Incentives <- stan.dataSpatialExplore$Incentives[which(stan.dataSpatialExplore$Exploit == 0)]
stan.dataSpatialExplore$N <- length(stan.dataSpatialExplore$t[which(stan.dataSpatialExplore$Exploit == 0)])

stan.dataSpatialExplore$N_cats <- length(unique(stan.dataSpatialExplore$dist_cat))

m_parallel <- cmdstan_model("SpatialGP.stan", cpp_options = list(stan_threads = TRUE))
fit_parallel <- m_parallel$sample(stan.dataSpatialExplore, chains = 2, parallel_chains = 2, threads_per_chain = 45, refresh = 1, iter_warmup = 1000 ,adapt_delta = 0.9, max_treedepth = 14, iter_sampling = 500)
fit <- rstan::read_stan_csv(fit_parallel$output_files())
save(fit, file = "fit_GPSpaceDisc")
...
values_coop_con <- matrix(NA, length(s$lp__), stan.dataSpatialExplore$N_cats )
for (draw in 1:length(s$lp__)) {
  values_coop_con[draw,] <- inv_logit(s$space_effects[draw,1,1,] )
}

values_coop_dist <- matrix(NA, length(s$lp__),  stan.dataSpatialExplore$N_cats )
for (draw in 1:length(s$lp__)) {
  values_coop_dist[draw,] <- inv_logit(s$space_effects[draw,1,2,] )
}

values_comp_con <- matrix(NA, length(s$lp__), stan.dataSpatialExplore$N_cats )
for (draw in 1:length(s$lp__)) {
  values_comp_con[draw,] <- inv_logit(s$space_effects[draw,2,1,] )
}

values_comp_dist <- matrix(NA, length(s$lp__),  stan.dataSpatialExplore$N_cats )
for (draw in 1:length(s$lp__)) {
  values_comp_dist[draw,] <- inv_logit($space_effects[draw,2,2,] )
}


#Group Incentives
plot(values_coop_con[1,], type = "n", ylab = "", xlab = "", bty = "n", ylim = c(0, 0.1), xaxt = "n")
mtext(side = 3, "Group Incentives", line = 0.5, cex = 1.2)
...
# plot a shaded region for 89% PI
shade( mu.PI , 1:stan.dataSpatialExplore$N_cats , col =  alpha(col.pal[2], alpha = 0.4))

# plot the MAP line, aka the mean mu for each weight
lines( 1:stan.dataSpatialExplore$N_cats , mu.mean, col = col.pal[2] )
...
# plot a shaded region for 89% PI
shade( mu.PI , 1:stan.dataSpatialExplore$N_cats , col =  alpha(col.pal[3], alpha = 0.4))

# plot the MAP line, aka the mean mu for each weight
lines( 1:stan.dataSpatialExplore$N_cats , mu.mean, col = col.pal[3] )
...
#Individual Incentives
plot(values_comp_con[1,], type = "n", ylab = "", xlab = "",bty = "n", ylim = c(0, 0.1), xaxt = "n", yaxt = "n")
abline(h = 0, lty = 2, col = "grey")
mtext(side = 3, "Individual Incentives", line = 0.5, cex = 1.2)
